...
st.subheader(
    "1. High Traffic Pages with Zero Key Events (Conversion Bottlenecks)"
)

if not df_blog.empty:
    page_col = (
        "Page path and screen class"
        if "Page path and screen class" in df_blog.columns
        else df_blog.columns[0]
    )
    ...
    if views_col in df_blog.columns:
        if events_col and events_col in df_blog.columns:
            bottlenecks = df_blog[
                (df_blog[views_col] > 100)
                & (df_blog[events_col].fillna(0) == 0)
            ]
        else:
            bottlenecks = df_blog.sort_values(
                by=views_col, ascending=False
            ).head(10)

        if not bottlenecks.empty:
            st.warning(
                f"Found {len(bottlenecks)} high-traffic pages producing zero conversions. Add CTAs or lead forms here first."
            )
            display_cols = [
                c
                for c in [page_col, views_col, events_col]
                if c and c in bottlenecks.columns
            ]
            st.dataframe(bottlenecks[display_cols].head(10), width="stretch")
        else:
            st.success(
                "No critical high-traffic zero-conversion bottlenecks found."
            )
else:
    st.info(
        "💡 Upload `01_Blog_GA4.csv` to detect high-traffic conversion bottlenecks."
    )
...
if not df_gsc.empty and "Impressions" in df_gsc.columns:
    st.markdown("---")
    st.subheader(
        "2. High Impression Terms with Low CTR (Title/Meta Opportunity)"
    )
    ...
    if "CTR" in df_gsc.columns:
        # Convert columns to numeric floats/ints (stripping out '%' or ',' if present)
        for col in ["Impressions", "CTR"]:
            if col in df_gsc.columns:
                ...
                df_gsc[col] = pd.to_numeric(df_gsc[col], errors="coerce").fillna(
                    0
                )

        # Filter low CTR opportunities safely
        low_ctr = df_gsc[
            (df_gsc["Impressions"] > 500) & (df_gsc["CTR"] < 2.0)
        ].sort_values(by="Impressions", ascending=False)

        if not low_ctr.empty:
            st.write(
                "These keywords get search impressions but low clicks. Rewrite page titles and meta descriptions:"
            )
            render_table_with_download(
                low_ctr.head(10),
                filename="low_ctr_keywords.csv",
                key="low_ctr",
            )
        else:
    ...
    if "Position" in df_gsc.columns:
        striking_distance = df_gsc[
            (df_gsc["Position"] >= 10) & (df_gsc["Position"] <= 20)
        ].sort_values(by="Impressions", ascending=False)
        if not striking_distance.empty:
            st.write(
                "Keywords sitting on page 2 of Google. Target these with internal links and content updates:"
            )
            render_table_with_download(
                striking_distance.head(10),
                filename="page2_keywords.csv",
                key="page2_kw",
            )
        else:
